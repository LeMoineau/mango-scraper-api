...
(0, vitest_1.describe)("array-utils", () => {
    ...
    (0, vitest_1.it)("should transform each item of array into new items", () => {
        const AN_ARRAY = [{ a: 1 }, { a: 2 }];
        const A_TRANSFORMATER = (a_item) => {
            return { b: a_item.a };
        };
        (0, vitest_1.expect)(array_utils_1.ArrayUtils.transformEachItemOf(AN_ARRAY, A_TRANSFORMATER)).toEqual([
            { b: 1 },
            { b: 2 },
        ]);
    });
    (0, vitest_1.it)("should not transform item of transformater return undefined", () => {
        const AN_ARRAY = [{ a: 1 }, { a: 2 }];
        const A_TRANSFORMATER = (_) => undefined;
        (0, vitest_1.expect)(array_utils_1.ArrayUtils.transformEachItemOf(AN_ARRAY, A_TRANSFORMATER)).toEqual([]);
    });
});
