    ...
    getManga(id) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (text_format_utils_1.TextFormatUtils.isNumber(id)) {
                $ = yield scraping_utils_1.ScrapingUtils.requestToCheerioPage(`${this.PAGE_URL}/node/${id}`);
            }
            else {
                $ = yield scraping_utils_1.ScrapingUtils.requestToCheerioPage(`${this.PAGE_URL}/manga/${id}`);
            }
            ...
            $("div#main .node-manga table tbody tr").each((i) => {
                ...
                chapters.push({
                    id: array_utils_1.ArrayUtils.getLastOf($(`${currentChapterPath} a`).attr("href").split("/")),
                    number: text_format_utils_1.TextFormatUtils.stringWithout($(`${currentChapterPath} a`).text(), mangaTitle),
                    title: $(`${currentChapterPath} a`).text(),
    ...
    getChapterPages(_, chapterId) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (text_format_utils_1.TextFormatUtils.isNumber(chapterId)) {
                $ = yield scraping_utils_1.ScrapingUtils.requestToCheerioPage(`${this.PAGE_URL}/node/${chapterId}`);
            }
            else {
                $ = yield scraping_utils_1.ScrapingUtils.requestToCheerioPage(`${this.PAGE_URL}/chapter/${chapterId}`);
            }
            let pages = $.html()
